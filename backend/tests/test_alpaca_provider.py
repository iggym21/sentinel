...
def test_get_price_history_sends_auth_headers():
    route = respx.get("https://paper-api.alpaca.markets/v2/stocks/AAPL/bars").mock(
        return_value=httpx.Response(200, json={"bars": []})
    )
    provider = AlpacaProvider(api_key="my-key", secret_key="my-secret", base_url="https://paper-api.alpaca.markets")
...
def test_get_price_snapshot_derives_change_and_avg_volume():
    respx.get("https://paper-api.alpaca.markets/v2/stocks/AAPL/snapshot").mock(
